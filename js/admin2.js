...
$(document).on("click", "#btnModificarProveedor", function () {
    ...
    .then(data => {
        ...
        const proveedor = data[0]; // Tomamos el primer registro del cursor

        Swal.fire({
            title: "Modificar Proveedor",
            html: `
                <input type="text" id="nombre" class="swal2-input" placeholder="Nombre" value="${proveedor.NOMBRE || ''}">
                <input type="text" id="identificacion" class="swal2-input" placeholder="Identificación" value="${proveedor.IDENTIFICACION || ''}">
            ...
            preConfirm: () => {
                ...
                if (!nombre || !identificacion || !direccion || !telefono) {
                    Swal.showValidationMessage("Todos los campos son obligatorios.");
                    return false;
                }

                return { id, nombre, identificacion, direccion, telefono };
            }
        }).then(result => {
            if (result.isConfirmed && result.value) {
                const { id, nombre, identificacion, direccion, telefono } = result.value;
