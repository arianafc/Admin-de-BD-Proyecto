...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $sql = "BEGIN FIDE_LOS_JAULES_RESERVAS_PKG.FIDE_RESERVAS_TB_INSTALACIONES_SP (:cursor, :tipo); END;";
        $stmt = oci_parse($conn, $sql);

        $cursor = oci_new_cursor($conn);
        oci_bind_by_name($stmt, ":cursor", $cursor, -1, OCI_B_CURSOR);
        oci_bind_by_name($stmt, ":tipo", $tipo, -1, SQLT_INT);
        ...
        if (empty($instalaciones)) {
            echo json_encode(["success" => false, "message" => "No hay instalaciones disponibles."]);
            exit;
        }
